...
MemoryWeaver 可以把“先给选项，再按用户选择执行”作为更安全的自主入口。当用户
意图不清时，assistant 不直接行动，而是给出一组经过 Harness 过滤的选项。用户
可以选择、拒绝或纠正。被选择的选项会变成 `ActionProposal`，而不是 verified
memory。

MemoryWeaver 也可以使用预测式上下文压缩：先预测用户可能的后续意图，再用真实
输入验证。猜中的预测只作为 route hint 或上下文压缩线索；猜不中的预测直接丢弃。

边界保持不变：

```text
预测 / 选项 -> candidate only
用户反馈 / 工具结果 -> evidence
Harness -> authority
```

预测不能直接写入 Layer 2，也不能直接晋升 Layer 3。详见
