"""LoCoMo-MC10 Hugging Face preview adapter check."""
...
from memoryweaver.external.locomo_mc10 import (
    DATASET_REPO_ID,
    build_locomo_mc10_episodes,
    fetch_locomo_mc10_preview,
    load_locomo_mc10_rows,
)
from memoryweaver.schema import Source


DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "validation" / "locomo-mc10-adapter-check"
...
def evaluate_rows(
    ...
    *,
    ...
    episodes = build_locomo_mc10_episodes(rows, split=split)
    raw_spans = [
        raw_span
    ...
    choice_counts = [
        int(episode.queries[0].metadata.get("num_choices") or 0)
        for episode in episodes
        if episode.queries
    ]
    metrics = {
        ...
        "choice_10_rate": round(sum(1 for count in choice_counts if count == 10) / len(choice_counts), 4)
        if choice_counts
        else 0.0,
        ...
        "temporal_signal_count": sum(
            1
            for episode in episodes
            for query in episode.queries
            if "temporal" in query.signal_types
        ),
    }
    hard_gates = {
        "sample_count": metrics["sample_count"] > 0,
        "query_answer_pair_coverage": metrics["query_answer_pair_coverage"] == 1.0,
        "choice_10_rate": metrics["choice_10_rate"] == 1.0,
...
def _readme(result: dict[str, Any]) -> str:
    ...
    return f"""# LoCoMo-MC10 Adapter Check
...
- choice_10_rate = {metrics['choice_10_rate']}
...
def run(
    ...
    *,
    ...
    if input_path is not None:
        rows = load_locomo_mc10_rows(input_path, sample_limit=sample_limit)
        metadata = {"input_path": str(input_path), "sample_limit": sample_limit}
        source_mode = "fixture"
    elif live:
        rows, metadata = fetch_locomo_mc10_preview(sample_limit=sample_limit)
