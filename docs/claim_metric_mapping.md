...
| Option and prediction candidates do not grant memory authority | option `promotion_allowed == false`, prediction `verified_memory_allowed == false`, unmatched predictions discarded, DeepSeek-style prefix hits retained only as route hints, high-risk options require confirmation | `tests/test_option_harness.py` | `docs/option_guided_predictive_harness.md` |
